...
如果没有接入IPV6网络，使用 `ipconfig` 命令得到的 IP 地址如下：
...
IPv6地址以 `fe80` 开头的，表明是一个内网地址，类似于IPv4 的 `192.168.1.100` ，外网是无法直接访问的。
...
因为 linux 和 mac 系统在默认下是使用 EUI-64 来生成 ipv6 地址，而windows是默认随机生成（都可以改）
