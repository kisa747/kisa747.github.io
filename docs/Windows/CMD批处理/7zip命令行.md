...
7zip的命令行程序是程序目录下的 `7z.exe`，并且依赖 `7z.dll`。
...
# -p123 设置密码为：123
# -mhe 加密header，也就是加密文件名。-mhe 与-p参数一起使用才有效。
```
...
# 默认 5，标准压缩；0 不压缩；9，极限压缩； 0、1、3、5、7、9共6个等级。
# 7z格式下，默认使用 LZMA2 算法。
```
...
`!` 使用通配符进行匹配。`@` 使用 listfile 文件进行匹配。可以将要匹配的规则写到一个文件中，用换行符隔开，文件需用utf-8编码。
...
>    
>    7z通配符与windows操作系统的不同。`*.*` 不匹配所有文件，而是匹配有扩展名的文件，要匹配所有文件，使用 `*` 。
...
实际测试压缩2万个文件，7z：92.55s，zip：96.99s。
