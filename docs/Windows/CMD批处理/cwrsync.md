...
cwrsync 是 rsync 在windows 上的实现，官方地址 [cwrsync](https://www.itefix.net/cwrsync/client/downloads) ，一般下载client 客户端即可。

## 关闭ACL功能

这个操作很重要，否则Windows之间同步权限会混乱，同步至Linux权限也会混乱。

在 `cwrsync` 安装目录的 `etc` 文件夹下创建 `fstab `文件，确保关闭ACL功能。内容如下：
...
需要确保Linux服务端也要安装有 `rsync` 软件，如果没有，手动安装

```sh
sudo apt update && sudo apt install rsync
```

设置免密登录SSH登录，需要设置私钥的权限，管理员权限运行以下命令，**删除所有继承的 ACE、赋予当前用户完全访问权限**。
