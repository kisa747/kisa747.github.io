...
参考：[使用Dism++安装系统](https://www.chuyu.me/zh-Hans/Document.html?file=Best/使用Dism++安装系统.md)
...
windows系统默认就安装了windowsRE，位置在`C:\Recovery\WindowsRE\Winre.wim`，windows的系统光盘内也有这个文件，经过哈希对比，两个文件是一致的，下面以提取Windows 10镜像为例。

将Windows 10镜像光盘挂载至 `H` 盘，以下均是管理员权限运行命令提示符。
