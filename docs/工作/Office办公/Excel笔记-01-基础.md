...
TRUNC(number, [num_digits]) 截去指定小数点后[num_digits]（位数）的小数部分（需要截尾数字，[取整精度]默认为 0）
...
返回 0 ≤X<1 的均匀分布随机实数，每次计算工作表时都将重新计算

​         无参数。a 与 b 之间的随机数： =RAND()*(b-a)+a

### 7、RANDBETWEEN(bottom, top)随机整数

返回位于指定的两个数（包含）之间的一个随机整数
