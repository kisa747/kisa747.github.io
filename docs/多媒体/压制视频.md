...
如果要一键批量压制，FFmpeg是更好的选择，缺点：由于 GPL 原因，无法内置 FDK AAC、NeroAAC 编码器。

官方FFmpeg下载地址：https://ffmpeg.org/download.html

x265下载地址：<http://msystem.waw.pl/x265/>

Mediainfo下载地址：<https://mediaarea.net/en/MediaInfo/Download/Windows>
...
# 默认的编码质量为 28（质量上约等于x264的 CRF 23，体积为x264的一半），值越小，质量越高。
# CRF 26 转换后，体积是iphon13 拍摄 4K 视频的一半。
# 转换为 mkv 格式才能保留 杜比HDR 信息。
...
av1编码器更新迅速，因此需要下载最新的 git master builds 。
...
# -preset 8 （默认值是 10）。参考nannan 的博客，设置为 4 比较合理，但是我的电脑只能设置到8，值再小的话，就会提示内存不足。
...
视频：推荐使用 x265 编码进行压制，添加 `-D 10` 参数 10bit 压制，默认质量 26 即可（x265默认值是28，x264默认的质量为23），得到码率约为 2.3M/s（帧率为24FPS，分辨率为1080P）。

10bit 压制质量更好，体积反而更小（约小2%吧），x265现在的速度约是x264的60%

音频：推荐使用 NeroAAC 编码压制，默认质量 q=5.0 即可（对应单声道码率约为87kbps，双声道约 175kbps，7.1声道约640kbps）。（注：q=5.5双声道约200kbps）

封装格式：推荐使用 MKV，Megui的mp4封装格式有问题，无法拖动快进。
