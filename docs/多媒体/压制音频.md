...
# 使用ffmpeg，即使不使用 -c:v copy 参数也能神奇的保留专辑封面。ffmpeg的压缩等级是0~12，默认是5。
...
1、libfdk_aac，FFmpeg能用的最好AAC 编码器，但是官方默认编译的 ffmpeg 中默认并没有包含 libfdk_aac，需要自行编译，或是下载包含 nonfree 包的编译版。
...
# 使用ffmpeg转换，可以转换flac格式，可以复制封面照片。
...
Windows 10 默认支持 flac、m4a（AAC、ALAC编码）、mp3 格式的内嵌封面。
