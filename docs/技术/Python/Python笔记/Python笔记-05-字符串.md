...
f-string 完全继承了 `str.format()` 的格式规范，在 `{表达式:格式规范}` 中添加格式控制：
...
### 详细语法
 ...
 参考：<https://docs.python.org/3/library/string.html#format-string-syntax>
...
* `type`【可选】格式化类型
  ...
  * *传入“整数类型”的参数*
    * `d`，十进制整数**（常用参数）**
    * `b`，将 10 进制整数自动转换成 2 进制
    * `c`，将 10 进制整数自动转换为其对应的 unicode 字符
    * `o`，将 10 进制整数自动转换成 8 进制
    * `x`，将 10 进制整数自动转换成 16 进制
    * `X`，将 10 进制整数自动转换成 16 进制
  * 传入`浮点型或小数类型`的参数
    * `f`，转换为浮点型（默认小数点后保留 6 位）**（常用参数）**
    * `%`，显示百分比（默认显示小数点后 6 位）
    * `e`，转换为科学计数法（小写 e）
    * `E`，转换为科学计数法（大写 E）
    * `F`，转换为浮点型（默认小数点后保留 6 位）
    * `g`，自动在 e 和 f 中切换
    * `G`，自动在 E 和 F 中切换
