...
* [廖雪峰 | Git教程](https://www.liaoxuefeng.com/wiki/0013739516305929606dd18361248578c67b8067c8c017b000)     详细讲解了git的原理、流程。
* [易百教程 | Git教程](https://www.yiibai.com/git/)  有常用命令的详解。
* [Git初阶使用笔记](<https://steinslab.io/archives/1520>)  推荐看看，里面有很多常用的实例。
...
# Windows命令提示符下使用git status命令，中文文件名会有乱码：
modified:   "\350\256\241\345\210\222\344\273\273\345\212\241/0-sync.cmd"
# 将core.quotepath设为false，git就不会对0x80以上的字符进行quote。中文显示正常。
git config --global core.quotepath false
...
git init
# 克隆一个远程的仓库
git clone git@github.com:kisa747/py_code.git

# git重要的命令
git pull # 更新本地仓库至最新改动
git add . # 添加当前目录下的所有文件
git commit -m "提交的信息" # 提交版本信息
git push # 这一步才是真正的上传
...
# 修改.gitignore后不生效，解决方法：
git rm -r --cached .  //删除暂存区所有追踪的文件、目录
git add .  //重新添加所有文件、目录至暂存区
...
# .gitignore 规则不生效。 .gitignore 只能忽略那些原来没有被track的文件，如果某些文件已经被纳入了版本管理中，则修改.gitignore是无效的。
...
尽管使用ssh 更为方便，但如果要使用https，当第一次push 时，会出现一个 `credential helperSelector`的提示。
...
Git共有三个级别的config文件，分别是**system、global和local**。在当前环境中，分别对应`%GitPath%\mingw64\etc\gitconfig`文件、`$home\.gitconfig`文件和`%RepoPath%\.git\config`文件。其中`%GitPath%`为Git的安装路径，`%RepoPath%`为某仓库的本地路径。所以system配置整个系统只有一个，global配置每个账户只有一个，而local配置和git仓库的数目相同，并且只有在仓库目录才能看到该配置。

>manager  若安装Git时安装了GitGUI，自动会在system级别中设置credential.helper为manager。并且不配置所处级别（system、global或者local）如何，一旦设置了manager，都优先使用该方式。若本地没有存储凭证，第一次push的时候，会弹出窗口，要求输入用户名密码（图1）。输入并验证成功后并将其存储至Windows的凭据管理器中（图2）。
>
>wincred  **推荐使用**。与manager很像，也存储在Windows的凭据管理器中，。如果本地没有账户信息，当push时会提示输入。如果输入正确也会记录在Windows的凭据管理器中。除此之外，该工具还可以使用命令行管理本地存储。**但是，经个人测试， 如果同一个host使用wincred存储多份凭证，只会有一个账户生效。**
