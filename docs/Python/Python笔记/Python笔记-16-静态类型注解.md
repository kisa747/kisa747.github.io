...
参考：[mypy文档](https://mypy.readthedocs.io/en/stable/) 、[Python官方文档：typing—类型注解支持](https://docs.python.org/zh-cn/3/library/typing.html) 
...
# list这三种写法效果一样。都表示：任意多元素，整数或字符串
# 一般采用第一种写法
...
# 鸭子类型，Sequence表示可以序列化对象（可以下标访问、可以切片访问），比如：list、tuple、str
...
# 方法1：忽略所有第三方库缺失的 types 包，在配置文件 pyproject.toml 中添加以下内容：
[tool.mypy]
