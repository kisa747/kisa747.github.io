...
```sh
# 在当期目录下创建新项目，类型为python库，构建后端为setuptools，如果不指定，默认为 uv_build
...
# 安装最新的 3.11 版
pyenv install 3.11
# 安装后的python没有包含 pip 包，需要手动安装
python3.11 -m ensurepip --upgrade

# 当前shell会话，切换至 python3.10
...
每次切换默认python版本后，pip 并不会跟随更改，需要手动强制重新安装一下 pip 到 user 就行了。
...
# 查看当前 python 版本
python3 -V
# 安装 python 3.11，需要安装full包，才会带pip包
...
# 在 .venv 目录创建虚拟环境，并将 pip、setuptools 升级到最新版本（python3.9新功能）
