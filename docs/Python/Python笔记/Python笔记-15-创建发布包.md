...
如果软件升级，需要手动修改，而且还要同步修改软件包内的版本号，如果版本号要带上 git 的commit 信息、日期等内容，会更麻烦。因此需要自动生成版本号。
...
| 状态                                              | 命名规则                                                                 | 示例                                                     |                                                                                        |
|:-----------------------------------------------:|:--------------------------------------------------------------------:|:------------------------------------------------------:| -------------------------------------------------------------------------------------- |
| no distance and clean（当前commit就在tag上，代码没有修改）    | `{tag}`                                                              | pyexcel-1.0.2-py3-none-any.whl                         |                                                                                        |
| distance and clean（当前commit不在tag上，代码没有修改）       | `{next_version}.dev{distance}+{scm letter}{revision hash}`           |                                                        |                                                                                        |
| no distance and not clean（当前commit就在tag上，代码有修改） | `{tag}+dYYYYMMDD`                                                    |                                                        |                                                                                        |
| distance and not clean（当前commit不在tag上，代码有修改）    | `{next_version}.dev{distance}+{scm letter}{revision hash}.dYYYYMMDD` | pyexcel-1.0.2.dev1+g7b09aab.d20221113-py3-none-any.whl | The next version is calculated by adding `1` to the last numeric component of the tag. |
