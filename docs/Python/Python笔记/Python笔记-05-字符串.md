...
```python
>>> i = 1
>>> print(f'{i:0>2d}.pdf')  # 格式化数字，保持2位，不足部分用0填充。
01.pdf
...
[python3 拼接字符串的7种方法](<https://www.pythontab.com/html/2018/pythonjichu_0730/1331.html>) 
...
可以使用贪婪模式 `\s+` 将所有空格变为1个空格。与 Excel 中的 `clean` + `trim` 两个组合函数效果差不多，除了单词之间的单个空格之外，移除文本中的所有空格、换行。

```python
>>> import re
>>> s = 'T-shirt\xa0\xa0\xa0\xa0\xa0短袖\n\n圆领衫,\u3000\xa0体恤衫\xa0买一件\t\t吧'
>>> re.sub('\s', ' ', s)
...
* 方法1：`str.format()`，比如：`'{0}, 成绩 {1:.1f}%'.format('小明', 17.125)`

```python
str.format(*args, **kwargs) # 基本语法
# 可变参数，意思可以解包序列，格式化各个元素，可选关键字用来通过key使用value
# 使用format() 方法还有一个好处，可以在表达式中作为整体使用。f-string就不行
...
>>> 'Coordinates: {latitude}, {longitude}'.format(latitude='37.24N', longitude='-115.81W')        #通过字典的key使用value值
