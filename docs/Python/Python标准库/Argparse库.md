...
argsparse是python的命令行解析的标准模块，内置于python，不需要额外安装。这个库可以让我们直接在命令行中就可以向程序中传入参数并让程序运行。
...
def main():
    ...
    arg_parser.add_argument('-m', '--merge', metavar='目录', help='合并文件夹下所有excel文件至一个excel文件')
