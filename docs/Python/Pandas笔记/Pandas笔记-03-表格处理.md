...
参考：[用Python完成Excel的常用操作](https://www.jianshu.com/p/9bc9f473dd22)

pandas系列学习（六）：数据聚合 https://blog.csdn.net/CoderPai/article/details/83577805
...
# 所以一般情况下，应该优先使用se.str.cat() 方法，可以使用 na_rep='' 参数完美解决问题。
df['date'] = df['year'].str.cat(df['month'], sep='-', na_rep='')


# 如果第一个参数others=None，会返回一个将整列串起来的字符串，与 str.join() 实现的效果一样。
