...
```python
# 读取csv文件，可以指定索引列、是否解析日期、编码，还可以额外指定infer_datetime_format=True，提高解析速度。
...
# df的 'date' 列位于第一列，且原来的2、3、4列会被丢弃。
df = pd.read_csv('1.csv', parse_dates={'date':[2,3,4]}, encoding='gbk')
# 写入csv文件，默认会写入索引列，可以指定编码格式、日期格式。
df.to_csv('3.csv', index=False, encoding='gbk', date_format='%Y/%m/%d')
```

## 读写excel文件

读取excel文件，参考：https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.read_excel.html

```python
# sheet_name参数为 0 时，表示仅读取第一个工作表；参数为列表 [0,1, 'sheet1']，表示读取第1、2个工作表；
# sheet_name参数为 'sheet1' 时，表示读取名称为 sheet1 的工作表。
# parse_dates参数默认为 False
...
with pd.ExcelWriter(xlsx_path, engine='xlsxwriter', date_format='YYYY-MM-DD') as writer:
    df.to_excel(writer, sheet_name='A股数据', index=False, float_format='%.4f', freeze_panes=(1,0))
    workbook = writer.book  # 获取工作簿
    worksheet = writer.sheets['A股数据']  # 获取工作表
...
# if_exists 默认选项是fail，如果表名已存在则抛出ValueError。
