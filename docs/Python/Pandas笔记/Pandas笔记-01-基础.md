...
注意：当对一个 df 复制操作后得到df1，如果再就地修改df1，pandas会提示 `链式赋值` 警告。应该尽量避免这样操作。
...
```python
lis = [['2018/01/02', 1, -2, 'dda', '东北风', '3-4级'],
 ['2018/01/01', 11, -2, '晴~多云', '东北风', '1-2级'],
 ['2018/01/01', 1, -6, '小雪~大雪', '东北风', '1-2级'],
 ['2018/01/04', -2, -10, '大雪~阴', '东北风', '1-2级'],
 ['2018/01/05', 0, -8, '多云~小雪', '南风', '1-2级'],
 ['2018/01/06', -2, -6, '中雪~小雪', '无持续风向', '微风'],
 ['2018/01/07', None, -5, '阴~多云', '西风', '1-2级'],
 ['2018/01/08', 1, -5, '多云~晴', '西风', '4-5级'],
 ['2018/01/09', 2, -5, '晴', '西风', '3-4级'],
 ['2018/01/10', 4, None, '晴', '西风', '3-4级'],
 ['2018/01/11', 2, -8, '晴', '东北风', '1-2级']]
...
# n=30 要抽取的30行。frac=0.8，就是抽取其中80%。random_state=1，可以取得重复数据。
...
df.isnull()  # 判断是否有缺失值
df.isnull().any()  # 判断每列是否有缺失值，any 默认参数时axis=0
...
# Series的字符串的矢量方法。replace 默认是正则表达式，
...
# 按照索引重新排序
df.sort_index(inplace=True)  # sort_index、sort_values排序，没有重复项，才可以使用 inplace=True 参数，否则会报错。
# 索引去重。DataFrame.drop_duplicates函数的subset参数无法设置索引列（index），只能使用这个方法去重后复制一个新的DataFrame。
...
# 访问它的原始数据，也就是转换为ndarray，会丢弃Index。
# 建议使用 df.to_numpy()
df.values
...
# 统计下某列中每个值出现的次数，注意这个方法作用于Series
...
# 可以传入一个函数，也可以传入一个字典，也可以是个Series
...
```python
# 使用map方法，如果文本中有缺失值，会报错。
# 因为 np.nan 属于 float类型。
...
面列出了 Pandas中 和时间日期相关常用的类以及创建方法。
